...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/02/27 14:59:27 by oabdalha          #+#    #+#             */
/*   Updated: 2017/06/20 19:43:20 by oabdalha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		color_pixel(int hue, int max)
{
	...
	rgb[0] = (0xDD / (HUE_DEG / 6)) * (HUE_DEG / 6);
	while (++i < hue)
	{
		if (i < (HUE_DEG / 6))
			rgb[1] += 0xDD / (HUE_DEG / 6);
		if (i >= (HUE_DEG / 6) && i < (2 * HUE_DEG / 6))
			rgb[0] -= 0xDD / (HUE_DEG / 6);
		if (i >= (2 * HUE_DEG / 6) && i < (3 * HUE_DEG / 6))
			rgb[2] += 0xDD / (HUE_DEG / 6);
	...
	return (((rgb[0] & 0xDD) << 16) | ((rgb[1] & 0xDD) << 8) | (rgb[2] & 0xDD));
}
